...
public class Texture implements Closeable {
  ...
  public enum WrapMode {
    CLAMP_TO_EDGE(GLES31.GL_CLAMP_TO_EDGE),
    MIRRORED_REPEAT(GLES31.GL_MIRRORED_REPEAT),
    REPEAT(GLES31.GL_REPEAT);
   ...
   */
  public enum Target {
    TEXTURE_2D(GLES31.GL_TEXTURE_2D),
    TEXTURE_EXTERNAL_OES(GLES11Ext.GL_TEXTURE_EXTERNAL_OES),
    TEXTURE_CUBE_MAP(GLES31.GL_TEXTURE_CUBE_MAP);
  ...
  public enum ColorFormat {
    LINEAR(GLES31.GL_RGBA8),
    SRGB(GLES31.GL_SRGB8_ALPHA8);
  ...
  public Texture(SampleRender render, Target target, WrapMode wrapMode, boolean useMipmaps) {
    ...
    int minFilter = useMipmaps ? GLES31.GL_LINEAR_MIPMAP_LINEAR : GLES31.GL_LINEAR;

    try {
      GLES31.glBindTexture(target.glesEnum, textureId[0]);
      GLError.maybeThrowGLException("Failed to bind texture", "glBindTexture");
      GLES31.glTexParameteri(target.glesEnum, GLES31.GL_TEXTURE_MIN_FILTER, minFilter);
      GLError.maybeThrowGLException("Failed to set texture parameter", "glTexParameteri");
      GLES31.glTexParameteri(target.glesEnum, GLES31.GL_TEXTURE_MAG_FILTER, GLES31.GL_LINEAR);
      GLError.maybeThrowGLException("Failed to set texture parameter", "glTexParameteri");

      GLES31.glTexParameteri(target.glesEnum, GLES31.GL_TEXTURE_WRAP_S, wrapMode.glesEnum);
      GLError.maybeThrowGLException("Failed to set texture parameter", "glTexParameteri");
      GLES31.glTexParameteri(target.glesEnum, GLES31.GL_TEXTURE_WRAP_T, wrapMode.glesEnum);
      GLError.maybeThrowGLException("Failed to set texture parameter", "glTexParameteri");
    } catch (Throwable t) {
      close();
      throw t;
