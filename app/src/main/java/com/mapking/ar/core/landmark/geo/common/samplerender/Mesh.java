...
public class Mesh implements Closeable {
    ...
    public enum PrimitiveMode {
        POINTS(GLES31.GL_POINTS),
        LINE_STRIP(GLES31.GL_LINE_STRIP),
        LINE_LOOP(GLES31.GL_LINE_LOOP),
        LINES(GLES31.GL_LINES),
        TRIANGLE_STRIP(GLES31.GL_TRIANGLE_STRIP),
        TRIANGLE_FAN(GLES31.GL_TRIANGLE_FAN),
    ...
    public Mesh(
        ...
        try {
            // Create vertex array
            GLES31.glGenVertexArrays(1, vertexArrayId, 0);
            GLError.maybeThrowGLException("Failed to generate a vertex array", "glGenVertexArrays");

            // Bind vertex array
            GLES31.glBindVertexArray(vertexArrayId[0]);
            GLError.maybeThrowGLException("Failed to bind vertex array object", "glBindVertexArray");

            if (indexBuffer != null) {
                GLES31.glBindBuffer(GLES31.GL_ELEMENT_ARRAY_BUFFER, indexBuffer.getBufferId());
            }

            for (int i = 0; i < vertexBuffers.length; ++i) {
                // Bind each vertex buffer to vertex array
                GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER, vertexBuffers[i].getBufferId());
                GLError.maybeThrowGLException("Failed to bind vertex buffer", "glBindBuffer");
                GLES31.glVertexAttribPointer(
                        i, vertexBuffers[i].getNumberOfEntriesPerVertex(), GLES31.GL_FLOAT, false, 0, 0);
...
//    }
    ...
    public void close() {
        if (vertexArrayId[0] != 0) {
            GLES31.glDeleteVertexArrays(1, vertexArrayId, 0);
    ...
    public void lowLevelDraw() {
        ...
        if (indexBuffer == null) {
            ...
            GLES31.glDrawArrays(primitiveMode.glesEnum, 0, numberOfVertices);
            GLError.maybeThrowGLException("Failed to draw vertex array object", "glDrawArrays");
        } else {
            GLES31.glDrawElements(
                    primitiveMode.glesEnum, indexBuffer.getSize(), GLES31.GL_UNSIGNED_INT, 0);
            GLError.maybeThrowGLException(
