...
public class PlaneRenderer {
  ...
  public void createOnGlThread(Context context, String gridDistanceTextureName) throws IOException {
    ...
    planeProgram = GLES31.glCreateProgram();
    GLES31.glAttachShader(planeProgram, vertexShader);
    GLES31.glAttachShader(planeProgram, passthroughShader);
    GLES31.glLinkProgram(planeProgram);
    GLES31.glUseProgram(planeProgram);
    ...
    planeXZPositionAlphaAttribute = GLES31.glGetAttribLocation(planeProgram, "a_XZPositionAlpha");

    planeModelUniform = GLES31.glGetUniformLocation(planeProgram, "u_Model");
    planeNormalUniform = GLES31.glGetUniformLocation(planeProgram, "u_Normal");
    planeModelViewProjectionUniform =
        GLES31.glGetUniformLocation(planeProgram, "u_ModelViewProjection");
    textureUniform = GLES31.glGetUniformLocation(planeProgram, "u_Texture");
    gridControlUniform = GLES31.glGetUniformLocation(planeProgram, "u_gridControl");
    planeUvMatrixUniform = GLES31.glGetUniformLocation(planeProgram, "u_PlaneUvMatrix");
  ...
  private void draw(float[] cameraView, float[] cameraPerspective, float[] planeNormal) {
    ...
    GLES31.glUniformMatrix4fv(planeModelUniform, 1, false, modelMatrix, 0);
    GLES31.glUniform3f(planeNormalUniform, planeNormal[0], planeNormal[1], planeNormal[2]);
    GLES31.glUniformMatrix4fv(
        planeModelViewProjectionUniform, 1, false, modelViewProjectionMatrix, 0);

    indexBuffer.rewind();
    GLES31.glDrawElements(
        GLES31.GL_TRIANGLE_STRIP, indexBuffer.limit(), GLES31.GL_UNSIGNED_SHORT, indexBuffer);
  ...
  public void drawPlanes(Collection<Plane> allPlanes, Pose cameraPose, float[] cameraPerspective) {
    ...
    GLES31.glUniform4fv(gridControlUniform, 1, GRID_CONTROL, 0);

    // Enable vertex arrays
    GLES31.glEnableVertexAttribArray(planeXZPositionAlphaAttribute);
    ...
    for (SortablePlane sortedPlane : sortedPlanes) {
      ...
      GLES31.glUniformMatrix2fv(planeUvMatrixUniform, 1, false, planeAngleUvMatrix, 0);

      draw(cameraView, cameraPerspective, normal);
    }

    // Clean up the state we set
    GLES31.glDisableVertexAttribArray(planeXZPositionAlphaAttribute);
    GLES31.glBindTexture(GLES31.GL_TEXTURE_2D, 0);
