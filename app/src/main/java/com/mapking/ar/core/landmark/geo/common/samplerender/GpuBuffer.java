...
class GpuBuffer {
  ...
  public GpuBuffer(int target, int numberOfBytesPerEntry, Buffer entries) {
    ...
    try {
      ...
      GLES31.glGenBuffers(1, bufferId, 0);
      GLError.maybeThrowGLException("Failed to generate buffers", "glGenBuffers");

      GLES31.glBindBuffer(target, bufferId[0]);
      GLError.maybeThrowGLException("Failed to bind buffer object", "glBindBuffer");

      if (entries != null) {
        entries.rewind();
        GLES31.glBufferData(
            target, entries.limit() * numberOfBytesPerEntry, entries, GLES31.GL_DYNAMIC_DRAW);
      }
  ...
  public void set(Buffer entries) {
    ...
    if (entries.limit() <= capacity) {
      GLES31.glBufferSubData(target, 0, entries.limit() * numberOfBytesPerEntry, entries);
  ...
  public void free() {
    if (bufferId[0] != 0) {
      GLES31.glDeleteBuffers(1, bufferId, 0);
