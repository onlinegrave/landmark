 ...
 */
package com.mapking.ar.core.landmark.geo.common.rendering;
...
public class ObjectRenderer {
  ...
  public void createOnGlThread(Context context, String objAssetName, String diffuseTextureAssetName)
    ...
    GLES31.glTexParameteri(
        GLES31.GL_TEXTURE_2D, GLES31.GL_TEXTURE_MIN_FILTER, GLES31.GL_LINEAR_MIPMAP_LINEAR);
    GLES31.glTexParameteri(GLES31.GL_TEXTURE_2D, GLES31.GL_TEXTURE_MAG_FILTER, GLES31.GL_LINEAR);
    GLUtils.texImage2D(GLES31.GL_TEXTURE_2D, 0, textureBitmap, 0);
    ...
    GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER, totalBytes, null, GLES31.GL_STATIC_DRAW);
    GLES31.glBufferSubData(
        GLES31.GL_ARRAY_BUFFER, verticesBaseAddress, 4 * vertices.limit(), vertices);
    GLES31.glBufferSubData(
        GLES31.GL_ARRAY_BUFFER, texCoordsBaseAddress, 4 * texCoords.limit(), texCoords);
    GLES31.glBufferSubData(
        GLES31.GL_ARRAY_BUFFER, normalsBaseAddress, 4 * normals.limit(), normals);
    GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER, 0);

    // Load index buffer
    GLES31.glBindBuffer(GLES31.GL_ELEMENT_ARRAY_BUFFER, indexBufferId);
    indexCount = indices.limit();
    GLES31.glBufferData(
        GLES31.GL_ELEMENT_ARRAY_BUFFER, 2 * indexCount, indices, GLES31.GL_STATIC_DRAW);
  ...
  private void compileAndLoadShaderProgram(Context context) throws IOException {
    ...
    final int vertexShader =
        ShaderUtil.loadGLShader(TAG, context, GLES31.GL_VERTEX_SHADER, VERTEX_SHADER_NAME);
    final int fragmentShader =
        ShaderUtil.loadGLShader(
            TAG, context, GLES31.GL_FRAGMENT_SHADER, FRAGMENT_SHADER_NAME, defineValuesMap);
    ...
    modelViewUniform = GLES31.glGetUniformLocation(program, "u_ModelView");
    modelViewProjectionUniform = GLES31.glGetUniformLocation(program, "u_ModelViewProjection");

    positionAttribute = GLES31.glGetAttribLocation(program, "a_Position");
    normalAttribute = GLES31.glGetAttribLocation(program, "a_Normal");
    texCoordAttribute = GLES31.glGetAttribLocation(program, "a_TexCoord");

    textureUniform = GLES31.glGetUniformLocation(program, "u_Texture");

    lightingParametersUniform = GLES31.glGetUniformLocation(program, "u_LightingParameters");
    materialParametersUniform = GLES31.glGetUniformLocation(program, "u_MaterialParameters");
    colorCorrectionParameterUniform =
        GLES31.glGetUniformLocation(program, "u_ColorCorrectionParameters");
    colorUniform = GLES31.glGetUniformLocation(program, "u_ObjColor");

    // Occlusion Uniforms.
    if (useDepthForOcclusion) {
      depthTextureUniform = GLES31.glGetUniformLocation(program, "u_DepthTexture");
      depthUvTransformUniform = GLES31.glGetUniformLocation(program, "u_DepthUvTransform");
      depthAspectRatioUniform = GLES31.glGetUniformLocation(program, "u_DepthAspectRatio");
    }
  ...
  public void draw(
    ...
    GLES31.glUniform4f(
        ...
        1.f);
    GLES31.glUniform4fv(colorCorrectionParameterUniform, 1, colorCorrectionRgba, 0);

    // Set the object color property.
    GLES31.glUniform4fv(colorUniform, 1, objColor, 0);

    // Set the object material properties.
    GLES31.glUniform4f(materialParametersUniform, ambient, diffuse, specular, specularPower);

    // Attach the object texture.
    GLES31.glActiveTexture(GLES31.GL_TEXTURE0);
    GLES31.glBindTexture(GLES31.GL_TEXTURE_2D, textures[0]);
    GLES31.glUniform1i(textureUniform, 0);
    ...
    if (useDepthForOcclusion) {
      ...
      GLES31.glUniform1i(depthTextureUniform, 1);

      // Set the depth texture uv transform.
      GLES31.glUniformMatrix3fv(depthUvTransformUniform, 1, false, uvTransform, 0);
      GLES31.glUniform1f(depthAspectRatioUniform, depthAspectRatio);
    }
    ...
    GLES31.glVertexAttribPointer(
        positionAttribute, COORDS_PER_VERTEX, GLES31.GL_FLOAT, false, 0, verticesBaseAddress);
    GLES31.glVertexAttribPointer(normalAttribute, 3, GLES31.GL_FLOAT, false, 0, normalsBaseAddress);
    GLES31.glVertexAttribPointer(
        texCoordAttribute, 2, GLES31.GL_FLOAT, false, 0, texCoordsBaseAddress);

    GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER, 0);

    // Set the ModelViewProjection matrix in the shader.
    GLES31.glUniformMatrix4fv(modelViewUniform, 1, false, modelViewMatrix, 0);
    GLES31.glUniformMatrix4fv(modelViewProjectionUniform, 1, false, modelViewProjectionMatrix, 0);

    // Enable vertex arrays
    GLES31.glEnableVertexAttribArray(positionAttribute);
    GLES31.glEnableVertexAttribArray(normalAttribute);
    GLES31.glEnableVertexAttribArray(texCoordAttribute);
    ...
    if (blendMode != null) {
      ...
      switch (blendMode) {
        case Shadow:
          ...
          GLES31.glBlendFunc(GLES31.GL_ZERO, GLES31.GL_ONE_MINUS_SRC_ALPHA);
          break;
        case AlphaBlending:
    ...
    GLES31.glDisableVertexAttribArray(positionAttribute);
    GLES31.glDisableVertexAttribArray(normalAttribute);
    GLES31.glDisableVertexAttribArray(texCoordAttribute);
