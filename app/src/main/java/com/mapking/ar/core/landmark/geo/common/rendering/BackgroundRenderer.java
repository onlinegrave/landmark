...
public class BackgroundRenderer {
  ...
  public void createOnGlThread(Context context, int depthTextureId) throws IOException {
    ...
    GLES31.glBindTexture(textureTarget, cameraTextureId);
    GLES31.glTexParameteri(textureTarget, GLES31.GL_TEXTURE_WRAP_S, GLES31.GL_CLAMP_TO_EDGE);
    GLES31.glTexParameteri(textureTarget, GLES31.GL_TEXTURE_WRAP_T, GLES31.GL_CLAMP_TO_EDGE);
    GLES31.glTexParameteri(textureTarget, GLES31.GL_TEXTURE_MIN_FILTER, GLES31.GL_LINEAR);
    GLES31.glTexParameteri(textureTarget, GLES31.GL_TEXTURE_MAG_FILTER, GLES31.GL_LINEAR);
    ...
    {
      int vertexShader =
          ShaderUtil.loadGLShader(TAG, context, GLES31.GL_VERTEX_SHADER, CAMERA_VERTEX_SHADER_NAME);
      int fragmentShader =
          ShaderUtil.loadGLShader(
              TAG, context, GLES31.GL_FRAGMENT_SHADER, CAMERA_FRAGMENT_SHADER_NAME);

      cameraProgram = GLES31.glCreateProgram();
      GLES31.glAttachShader(cameraProgram, vertexShader);
      GLES31.glAttachShader(cameraProgram, fragmentShader);
      GLES31.glLinkProgram(cameraProgram);
      GLES31.glUseProgram(cameraProgram);
      cameraPositionAttrib = GLES31.glGetAttribLocation(cameraProgram, "a_Position");
      cameraTexCoordAttrib = GLES31.glGetAttribLocation(cameraProgram, "a_TexCoord");
      ShaderUtil.checkGLError(TAG, "Program creation");

      cameraTextureUniform = GLES31.glGetUniformLocation(cameraProgram, "sTexture");
    ...
    // Load render depth map shader.
    {
      int vertexShader =
          ShaderUtil.loadGLShader(
              TAG, context, GLES31.GL_VERTEX_SHADER, DEPTH_VISUALIZER_VERTEX_SHADER_NAME);
      int fragmentShader =
          ShaderUtil.loadGLShader(
              TAG, context, GLES31.GL_FRAGMENT_SHADER, DEPTH_VISUALIZER_FRAGMENT_SHADER_NAME);

      depthProgram = GLES31.glCreateProgram();
      GLES31.glAttachShader(depthProgram, vertexShader);
      GLES31.glAttachShader(depthProgram, fragmentShader);
      GLES31.glLinkProgram(depthProgram);
      GLES31.glUseProgram(depthProgram);
      depthPositionAttrib = GLES31.glGetAttribLocation(depthProgram, "a_Position");
      depthTexCoordAttrib = GLES31.glGetAttribLocation(depthProgram, "a_TexCoord");
      ShaderUtil.checkGLError(TAG, "Program creation");

      depthTextureUniform = GLES31.glGetUniformLocation(depthProgram, "u_DepthTexture");
      ShaderUtil.checkGLError(TAG, "Program parameters");
    }
  ...
  private void draw(boolean debugShowDepthMap) {
    ...
    if (debugShowDepthMap) {
      ...
      GLES31.glUseProgram(depthProgram);
      GLES31.glUniform1i(depthTextureUniform, 0);

      // Set the vertex positions and texture coordinates.
      GLES31.glVertexAttribPointer(
          depthPositionAttrib, COORDS_PER_VERTEX, GLES31.GL_FLOAT, false, 0, quadCoords);
      GLES31.glVertexAttribPointer(
          depthTexCoordAttrib, TEXCOORDS_PER_VERTEX, GLES31.GL_FLOAT, false, 0, quadTexCoords);
      GLES31.glEnableVertexAttribArray(depthPositionAttrib);
      GLES31.glEnableVertexAttribArray(depthTexCoordAttrib);
    } else {
      GLES31.glBindTexture(GLES11Ext.GL_TEXTURE_EXTERNAL_OES, cameraTextureId);
      GLES31.glUseProgram(cameraProgram);
      GLES31.glUniform1i(cameraTextureUniform, 0);

      // Set the vertex positions and texture coordinates.
      GLES31.glVertexAttribPointer(
          cameraPositionAttrib, COORDS_PER_VERTEX, GLES31.GL_FLOAT, false, 0, quadCoords);
      GLES31.glVertexAttribPointer(
          cameraTexCoordAttrib, TEXCOORDS_PER_VERTEX, GLES31.GL_FLOAT, false, 0, quadTexCoords);
      GLES31.glEnableVertexAttribArray(cameraPositionAttrib);
      GLES31.glEnableVertexAttribArray(cameraTexCoordAttrib);
    }
    ...
    if (debugShowDepthMap) {
      GLES31.glDisableVertexAttribArray(depthPositionAttrib);
      GLES31.glDisableVertexAttribArray(depthTexCoordAttrib);
    } else {
      GLES31.glDisableVertexAttribArray(cameraPositionAttrib);
      GLES31.glDisableVertexAttribArray(cameraTexCoordAttrib);
    }
